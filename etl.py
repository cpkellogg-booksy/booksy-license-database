...
# 2. Barbers (Generic 30 Cols to catch shifted data)
headers_generic = [f"col_{i}" for i in range(30)]
...
def parse_and_standardize(addr_str):
    ...
    suffix_mapping = {
        ...
        'LANE': 'LN', 'CIRCLE': 'CIR', 'COURT': 'CT', 'PARKWAY': 'PKWY', 'HIGHWAY': 'HWY', 
        'PLACE': 'PL', 'TRAIL': 'TRL', 'SQUARE': 'SQ', 'LOOP': 'LOOP', 'WAY': 'WAY', 'CAUSEWAY': 'CSWY'
    }
    unit_mapping = {
        'SUITE': 'STE', 'STE': 'STE', 'UNIT': 'STE', 'SHOP': 'STE', 'APARTMENT': 'APT', 
        'APT': 'APT', 'ROOM': 'RM', 'BUILDING': 'BLDG', 'BLDG': 'BLDG', 'FLOOR': 'FL'
    }
...
def load_bronze_layer(engine):
    ...
    # 2. Barbers (Messy - Using Generic Headers)
    print(f"\n   Downloading Barbers (Generic Schema)...")
    first_chunk = True
    for chunk in pd.read_csv(url_barber_ce, chunksize=chunk_size, header=None, names=headers_generic, 
                             storage_options={'User-Agent': 'Mozilla/5.0'}, encoding='ISO-8859-1', on_bad_lines='skip'):
        if first_chunk:
...
def transform_gold_layer(engine):
    ...
    # 2. Read Barbers (Generic Columns)
    # We select the columns we identified from the diagnostic
    # col_0 = Address (Maybe null)
    # col_1 = City
    # col_2 = State
    # col_3 = Zip
    # col_13 = License Number (Contains Code)
    query_barber = """
    SELECT col_0 as address_line_1, col_1 as city, col_2 as state, col_3 as zip, col_13 as license_number
    FROM florida_barbers_bronze
    WHERE col_2 = 'FL' -- Ensure we only get Florida rows
    """
    df_b = pd.read_sql(query_barber, engine)
    print(f"   Loaded {len(df_b)} Raw Barber records.")
    
    # Barber Cleanup
    # Extract Code from License Number (e.g., 'BB' from 'BB1234')
    df_b['occupation_code'] = df_b['license_number'].astype(str).str[:2]
    
    # Handle Missing Addresses: If address is null, use City as placeholder to allow count (but it won't be mappable)
    df_b['address_line_1'] = df_b['address_line_1'].fillna("UNKNOWN ADDRESS")
    
    # Deduplicate (One license per person)
    ...
    # Barber Flags
    df['is_barber'] = df['occupation_code'].isin(['BB', 'BR', 'BA']).astype(int)
    df['is_barbershop'] = df['occupation_code'].isin(['BS']).astype(int)
